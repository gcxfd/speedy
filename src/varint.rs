  ...
  ($reader:ident, $read_u8:ident, $read_bytes:ident) => {{
    ...
    macro_rules! read {
      ($count:expr) => {{
        ...
        {
          let slice =
            unsafe { std::slice::from_raw_parts_mut(&mut value as *mut u64 as *mut u8, $count) };
  ...
  for n in 0..64 {
    ...
    for &value in &values {
      ...
      let serialized_le = value
        ...
        .unwrap();
      let deserialized =
        VarInt64::read_from_buffer_with_ctx(crate::Endianness::LittleEndian, &serialized_le)
          .unwrap();
